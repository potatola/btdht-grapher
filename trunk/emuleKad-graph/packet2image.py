...
class Analyser:
    ...
    def __init__(self, canvas=None, main_window=None, type=1):
        ...
        if canvas != None:
            ...
            example_node.setSelfDate(_fromUtf8('节点类型'), _fromUtf8('发送过请求的节点'))
            ...
            example_node.setSelfDate(_fromUtf8('节点类型'), _fromUtf8('出现在应答信息中的节点'))
            self.canvas.scene().addItem(example_node)
            ...
            example_node.setSelfDate(_fromUtf8('节点类型'), _fromUtf8('只返回了peers信息的节点'))
            self.canvas.scene().addItem(example_node)
            example_node.setPos(__builtin__.__dict__['Scene_width'], 50)
            example_node = Node(self.canvas, Node_color['peers'], self.main_window)
            example_node.setSelfDate(_fromUtf8('节点类型'), _fromUtf8('返回资源信息的节点'))
            self.canvas.scene().addItem(example_node)
